import random
from .factions import Faction
from dataclasses import dataclass

# TODO temp
# "enum" for stat values
class TileStat:
    HIGH = 5
    MEDIUM = 3
    LOW = 1

# holder of the 4 stats on tiles
@dataclass
class TileStatBlock:
    attractiveness: int
    defense       : int
    fertility     : int
    resources     : int

# colony class, only used by tiles
class Colony:
    def __init__(self, name: str, stats: TileStatBlock):
        self.name = name
        self.stats = stats

# base tile class
class Tile:
    def __init__(self, terrain: str, x: int, y: int, faction: Faction | None, colony: Colony | None, gfx: str):
        self.terrain = terrain
        self.x, self.y = x, y
        self.faction = faction
        self.colony = colony
        self.graphic = gfx

        # generate stats based on terrain
        stats = TileStatBlock(0, 0, 0, 0)
        nudge: bool = True
        
        match terrain:
            case "TILE_BEACH":
                stats.attractiveness = TileStat.HIGH
                stats.defense = TileStat.LOW
                stats.fertility = TileStat.HIGH
                stats.resources = TileStat.MEDIUM
            case "TILE_PLAINS":
                stats.attractiveness = TileStat.HIGH
                stats.defense = TileStat.MEDIUM
                stats.fertility = TileStat.HIGH
                stats.resources = TileStat.LOW
            case "TILE_FOREST":
                stats.attractiveness = TileStat.LOW
                stats.defense = TileStat.HIGH
                stats.fertility = TileStat.MEDIUM
                stats.resources = TileStat.HIGH
            case "TILE_MOUNTAINS":
                stats.attractiveness = TileStat.MEDIUM
                stats.defense = TileStat.HIGH
                stats.fertility = TileStat.LOW
                stats.resources = TileStat.HIGH
            case _:
                nudge = False
        
        if nudge:
            # randomly nudge the tile's stats
            stats.attractiveness += random.randint(-1, 1)
            stats.defense += random.randint(-1, 1)
            stats.fertility += random.randint(-1, 1)
            stats.resources += random.randint(-1, 1)

        # assign stats to self and colony
        self.stats = stats

        if self.colony is not None:
            self.colony.stats = self.stats

# TODO unused, maybe get rid of this
class BaseTile(Tile):
    def __init__(self, x: int, y: int, colony: Colony | None):
        super().__init__('TILE_BASE', x, y, colony, 'base')

# all types of tiles generated by the game
class PlainsTile(Tile):
    def __init__(self, x: int, y: int, faction: Faction | None, colony: Colony | None):
        super().__init__('TILE_PLAINS', x, y, faction, colony, 'grass')

class FarmlandTile(Tile):
    def __init__(self, x: int, y: int, faction: Faction | None, colony: Colony | None):
        super().__init__('TILE_FARMLAND', x, y, faction, colony, 'farmland')

class ForestTile(Tile):
    def __init__(self, x: int, y: int, faction: Faction | None, colony: Colony | None):
        super().__init__('TILE_FOREST', x, y, faction, colony, 'forest')

class MountainTile(Tile):
    def __init__(self, x: int, y: int, faction: Faction | None, colony: Colony | None):
        super().__init__('TILE_MOUNTAINS', x, y, faction, colony, 'mountain')

class BeachTile(Tile):
    def __init__(self, x: int, y: int, faction: Faction | None, colony: Colony | None):
        super().__init__('TILE_BEACH', x, y, faction, colony, 'sand')

class OceanTile(Tile):
    def __init__(self, x: int, y: int, faction: Faction | None, colony: Colony | None):
        super().__init__('TILE_OCEAN', x, y, faction, colony, 'water')

class DeepOceanTile(Tile):
    def __init__(self, x: int, y: int, faction: Faction | None, colony: Colony | None):
        super().__init__('TILE_DEEP_OCEAN', x, y, faction, colony, 'deep_water')